...
public class CarouselView : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public float pixelPerUnit = 100f;//每一百像素单位
    [Range(0.85f, 0.99f)]
    ...
    private Queue<float> m_velocityHistory = new(10);//优化松手速度计算,暂定取10帧
    ...
    void Start()
    {
        ...
        //测试数据,后期计划封装到由inspector控制
        List<ListItemData> testData = new();
    ...
    public void SetData(List<ListItemData> data)
    {
        ...
        this.enabled = canScroll; //TODO:可以考虑取消限制

        m_scrollPhysics.SetBounds(0, m_allData.Count - 1);

        int startIndex = 0;
        if (!canScroll && m_allData.Count > 0)//不可滑动时找出中心元素
        {
            startIndex = (m_allData.Count - 1) / 2;
        }

        ScrollToIndex(startIndex, false);//初始化页面
    }
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        m_velocityHistory.Enqueue(frameVelocity);
    }
    ...
    private void UpdateAllViewsData()
    {
        for (int i = 0; i < 7; i++)
        {
            ...
            if (dataIndex < 0 || dataIndex >= m_allData.Count)//起始和最后数据的边界外item不显示
            {
